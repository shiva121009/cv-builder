...
function Score() {
  ...
  useEffect(() => {
    const fetchScore = async () => {
      const cached = localStorage.getItem("resume_score")
      if (cached) {
        setResumeScore(JSON.parse(cached))
      } else {
        const token = localStorage.getItem("access_token")
        if (!token) return
        try {
          const res = await fetch("https://your-api.com/resume/scores", {
            headers: {
          ...
          if (data?.results?.length > 0) {
            ...
            setResumeScore({ score: latest.score })
          }
        } catch (err) {
  ...
  const scoreValue = resumeScore?.score || 62
      ...
      <div className={Styles.body}>
        <div className={Styles.left}>
          <div className={Styles.dashboardSide}>
            ...
            <div className={Styles.dashboardHeading}>TOP FIXES</div>
            <ul className={Styles.List}>
              ...
              <Link href="/"><li className={Styles.subtreeSection}>
                ...
                <div className={Styles.ScoreBadgeLock}><FaLock /></div>
              </li></Link>
            ...
            <div className={Styles.showAll} style={{ cursor: 'pointer' }}>
              12 more issues <FaPlusSquare />
            </div>

            <div className={Styles.dashboardHeading}>COMPLETES</div>
            <ul className={Styles.List}>
              <Link href="/score"><li className={Styles.subtreeSection}>
                <div className={Styles.navTab}>Dates</div>
                <div className={Styles.ScoreBadge}>0</div>
            ...
            <div className={Styles.showAll} style={{ cursor: 'pointer' }}>
              1 MORE CHECKS <FaPlusSquare />
            </div>

            <div className={Styles.separator}></div>

            <div className={Styles.dashboardHeading}>TOOLS</div>
            <ul className={Styles.List}>
              <Link href="/components/notAvailable"><li className={Styles.subtreeSection}>
            ...
            <li className={Styles.buyProTabFixed} style={{ color: "black" }}>
              <Link href="/Subscription">
                <div className={Styles.navTabText}>Unlock full report</div>
              </Link>
              <div className={Styles.navTabValue}><FaStar /></div>
            </li>
          </div>
          ...
          <div className={Styles.preview}>
            ...
            <div className={Styles.greetingSection}>
              ...
              <div className={Styles.howItWork}>
                <Link href="/components/helpCenter"><button>HOW IT WORKS</button></Link>
              </div>
            </div>
            ...
            <div className={`${Styles.previewContainer}`}>
              <div className="max-w-md mx-auto bg-white pt-11 pr-0 pl-0 rounded-xl shadow space-y-6 font-sans text-gray-800">
                <h2 className="text-lg font-semibold leading-snug">
                  Your resume scored <span className="text-indigo-600 font-bold">{scoreValue}</span> out of 100.
                </h2>
                ...
                <div className="relative h-3 rounded-full bg-gradient-to-r from-red-500 via-yellow-300 via-yellow-400 via-green-400 to-green-600 mb-8">
                  ...
                  <div
                    ...
                    style={{ left: '90%' }}
                  ></div>
                  <div className="absolute top-full mt-3 left-[90%] -translate-x-1/2 text-xs text-gray-800 font-semibold">
                    TOP RESUMES
                  </div>
                </div>

                <div className="flex items-start space-x-3 bg-yellow-50 border border-yellow-200 p-4 mt-20 rounded-lg">
                  <svg className="w-5 h-5 text-yellow-500 mt-0.5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 352 512" fill="currentColor">
                    <path d="M96.06 454.35c.01 6.29 1.87 12.45 5.36 17.69l17.09 25.69a31.99 31.99 0 0 0 26.64 14.28h61.71a31.99 0 0 0 26.64-14.28l17.09-25.69a31.989 31.989 0 0 0 5.36-17.69l.04-38.35H96.01l.05 38.35zM0 176c0 44.37 16.45 84.85 43.56 115.78 16.52 18.85 42.36 58.23 52.21 91.45.04.26.07.52.11.78h160.24c.04-.26.07-.51.11-.78 9.85-33.22 35.69-72.6 52.21-91.45C335.55 260.85 352 220.37 352 176 352 78.61 272.91-.3 175.45 0 73.44.31 0 82.97 0 176zm176-80c-44.11 0-80 35.89-80 80 0 8.84-7.16 16-16 16s-16-7.16-16-16c0-61.76 50.24-112 112-112 8.84 0 16 7.16 16 16s-7.16 16-16 16z" />
                  </svg>
        ...
        <div className={Styles.right}>
          <div className={Styles.pdfbody}>
            <div className={Styles.pdfFeatures}>
              <ul className={Styles.list}>
             <Link href="/Subscription">   <li className={Styles.listItem}><FaStar style={{ color: "#48edcd" }} /><p>GET PRO</p></li></Link>
                {/* <li className={Styles.listItem}><BiAnalyse /><p>LINE ANALYSIS</p></li>
                <li className={Styles.listItem}><FaMagic /><p>MAGIC WRITE</p></li> */}
              </ul>
            </div>
